...
import { Link } from 'react-router-dom';
import Navbar from '../components/Navbar';
import skincareData from "../assets/data/skinproducts.json"; // Importing the products data
...
export default function Skincare() {
  ...
  useEffect(() => {
    ...
    const skintypes = [...new Set(skincareData.map(product => product.skintype))].map((skintype, index) => ({ id: index, label: skintype }));
    const skinconcerns = [...new Set(skincareData.map(product => product.skinconcern))].map((skinconcern, index) => ({ id: index, label: skinconcern }));
