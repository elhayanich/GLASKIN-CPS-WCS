...
export default function Haircare() {
  const [haircareProducts] = useState(hairproducts); // Directly setting the products
  const [filteredHairtypes, setFilteredHairtypes] = useState([]);
  const [filteredHairconcerns, setFilteredHairconcerns] = useState([]);
  const [selectedHairtype, setSelectedHairtype] = useState('');
  const [selectedHairconcern, setSelectedHairconcern] = useState('');
  ...
  useState(() => {
    const hairtypes = [...new Set(haircareProducts.map(product => product.hairtype))].map((hairtype, index) => ({ id: index, label: hairtype }));
    const hairconcerns = [...new Set(haircareProducts.map(product => product.hairconcern))].map((hairconcern, index) => ({ id: index, label: hairconcern }));
